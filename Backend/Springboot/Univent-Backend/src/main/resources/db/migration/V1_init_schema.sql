...
-- Create indexes
CREATE INDEX idx_reviews_college_program ON reviews(college_id, program_id);
CREATE INDEX idx_reviews_created_at ON reviews(created_at DESC);
CREATE INDEX idx_reviews_status ON reviews(status);
CREATE INDEX idx_reviews_overall_rating ON reviews(overall_rating DESC);
CREATE INDEX idx_colleges_avg_rating ON colleges(average_rating DESC);
CREATE INDEX idx_colleges_total_reviews ON colleges(total_reviews DESC);
CREATE INDEX idx_colleges_search ON colleges USING GIN(
    to_tsvector('english', COALESCE(name, '') || ' ' || COALESCE(city, '') || ' ' || COALESCE(state, ''))
    );
CREATE INDEX idx_college_programs_lookup ON college_programs(college_id, program_id);
